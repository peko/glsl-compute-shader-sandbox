...
int pindex; // The pindex points to the slot in the vectors/arrays that contains
            // data concerning the current processor, i.e pindex = (rank *
            // part_size)
 ...
 */
double generate_rand_ex() { return 2 * generate_rand() - 1; }
 ...
 *
 */
void BH_set_location_of_subcells(Cell *cell, double width, double heigth,
                                 double depth) {
...
void BH_compute_force_from_cell(Cell *cell, int index) {
  ...
  // Resolve forces in each direction
  force[index - pindex].fx +=
      f * ((position[cell->index].px - position[index].px) / d);
  force[index - pindex].fy +=
      f * ((position[cell->index].py - position[index].py) / d);
  force[index - pindex].fz +=
      f * ((position[cell->index].pz - position[index].pz) / d);
}
...
void BH_print_octtree_ex(Cell *cell, int level, int cell_no) {
  ...
  if (cell->no_subcells == 0 && cell->index != -1) {
    BH_print_spaces(level);
    printf("position[%d] = %.2f, %.2f, %.2f; cell-location = %.2f, %.2f, %.2f, "
           "mass = %.2f;\n",
           cell->index, position[cell->index].px, position[cell->index].py,
           position[cell->index].pz, cell->x, cell->y, cell->z,
           mass[cell->index]);
  } else {
 ...
 * for debugging
 */
void BH_print_octtree(Cell *cell) { BH_print_octtree_ex(cell, 0, 0); }
...
void print_space() {
  ...
  printf("\n\n Space with %d bodies \n", N);
  for (i = 0; i < N; i++) {
    printf("bodies%d: mass = %.2f, px=%.2f, py=%.2f, pz=%.2f, vx=%.4f, "
           "vy=%.4f, vz=%.4f\n",
           i, mass[i], position[i].px, position[i].py, position[i].pz,
...
void run_simulation() {
  ...
  if (rank == 0)
    printf("\nRunning simulation for %d bodies with %d iterations, and DELTAT "
           "= %f..\n\n",
           N, TIME, DELTAT);
  ...
  MPI_Scatter(ivelocity, part_size, MPI_VELOCITY, velocity, part_size,
              MPI_VELOCITY, 0, MPI_COMM_WORLD);
